...
def rip(con):
	...
	for i in range(len(ip_id)):
		print(f"\tID: {ip_id[i]} Wildcard: {wildcards[i]}")
	cmd=["conf t", "router rip","ver 2", "exit",f"router rip {len(known_routers)}"]
	for i in range(len(ip_id)):
...
def eigrp(con):
	...
	for i in range(len(ip_id)):
		print(f"\tID: {ip_id[i]} Wildcard: {wildcards[i]}")
	cmd=["conf t","router eigrp 90","exit",	f"router eigrp {len(known_routers)}"]
	for i in range(len(ip_id)):
		cmd.append(f"net {ip_id[i]}")
	cmd.append("ver 90")
	cmd.append("end")
