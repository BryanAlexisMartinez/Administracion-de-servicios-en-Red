...
def ping(host,result):
    ...
    res.terminate()
    if r:
        msg=f"{GREEN} Con respuesta [✓]{END}"
    else:
        msg=f"{RED} Sin respuesta [x]{END}"
    result.append([r,f"{YELLOW} Enviar datos a: {host.ljust(15)} {msg}",host,output.split("\n")[1]])
...
def is_host_up(srcs,host,result):
    ...
    if p is None:
        result.append([False,f"{YELLOW} Enviar datos a: {host.ljust(15)} {RED} Sin respuesta [x]{END}",host,None])
    else:
        result.append([True,f"{YELLOW} Enviar datos a: {host.ljust(15)} {GREEN} Con respuesta [✓]{END}",host,p.getlayer(IP).ttl])
